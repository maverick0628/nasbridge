...
export function register(server, client) {
    ...
    // SMB Shares
    // =========================================================================
    server.tool("smb_share_list", "List all SMB shares", {}, async () => jsonContent(await client.get("/sharing/smb")));
    server.tool("smb_share_get", "Get an SMB share by ID", { id: z.number().describe("SMB share ID") }, async ({ id }) => jsonContent(await client.get(`/sharing/smb/id/${id}`)));
    server.tool("smb_share_create", "Create a new SMB share", {
        ...
        return jsonContent(await client.post("/sharing/smb", body));
    });
    server.tool("smb_share_update", "Update an existing SMB share", {
        ...
        return jsonContent(await client.put(`/sharing/smb/id/${id}`, body));
    });
    server.tool("smb_share_delete", "Delete an SMB share", {
        id: z.number().describe("SMB share ID"),
        confirm: z.boolean().describe("Must be true to confirm deletion"),
    }, async ({ id, confirm }) => {
        if (!confirm)
            return jsonContent({ error: "Deletion not confirmed" });
        return jsonContent(await client.delete(`/sharing/smb/id/${id}`));
    });
    // =========================================================================
    // SMB Global Config
    // =========================================================================
    server.tool("smb_config", "Get global SMB configuration", {}, async () => jsonContent(await client.get("/smb")));
    server.tool("smb_config_update", "Update global SMB configuration", {
        ...
        const body = clean(params);
        return jsonContent(await client.put("/smb", body));
    });
    // =========================================================================
    // NFS Shares
    // =========================================================================
    server.tool("nfs_share_list", "List all NFS shares/exports", {}, async () => jsonContent(await client.get("/sharing/nfs")));
    server.tool("nfs_share_get", "Get an NFS share by ID", { id: z.number().describe("NFS share ID") }, async ({ id }) => jsonContent(await client.get(`/sharing/nfs/id/${id}`)));
    server.tool("nfs_share_create", "Create a new NFS share/export", {
        ...
        const body = clean(params);
        return jsonContent(await client.post("/sharing/nfs", body));
    });
    server.tool("nfs_share_update", "Update an existing NFS share/export", {
        ...
        const body = clean(rest);
        return jsonContent(await client.put(`/sharing/nfs/id/${id}`, body));
    });
    server.tool("nfs_share_delete", "Delete an NFS share/export", {
        id: z.number().describe("NFS share ID"),
        confirm: z.boolean().describe("Must be true to confirm deletion"),
    }, async ({ id, confirm }) => {
        if (!confirm)
            return jsonContent({ error: "Deletion not confirmed" });
        return jsonContent(await client.delete(`/sharing/nfs/id/${id}`));
    });
    // =========================================================================
    // NFS Global Config
    // =========================================================================
    server.tool("nfs_config", "Get global NFS configuration", {}, async () => jsonContent(await client.get("/nfs")));
    server.tool("nfs_config_update", "Update global NFS configuration", {
        ...
        const body = clean(params);
        return jsonContent(await client.put("/nfs", body));
    });
    server.tool("nfs_client_count", "Get the number of connected NFS clients", {}, async () => jsonContent(await client.get("/nfs/client_count")));
    // =========================================================================
    // iSCSI — Global Config
    // =========================================================================
    server.tool("iscsi_global_config", "Get iSCSI global configuration", {}, async () => jsonContent(await client.get("/iscsi/global")));
    server.tool("iscsi_global_config_update", "Update iSCSI global configuration", {
        ...
        const body = clean(params);
        return jsonContent(await client.put("/iscsi/global", body));
    });
    // =========================================================================
    // iSCSI — Targets
    // =========================================================================
    server.tool("iscsi_target_list", "List all iSCSI targets", {}, async () => jsonContent(await client.get("/iscsi/target")));
    server.tool("iscsi_target_create", "Create an iSCSI target", {
        ...
        const body = clean(params);
        return jsonContent(await client.post("/iscsi/target", body));
    });
    server.tool("iscsi_target_update", "Update an iSCSI target", {
        ...
        const body = clean(rest);
        return jsonContent(await client.put(`/iscsi/target/id/${id}`, body));
    });
    server.tool("iscsi_target_delete", "Delete an iSCSI target", {
        id: z.number().describe("Target ID"),
        ...
        if (!confirm)
            return jsonContent({ error: "Deletion not confirmed" });
        return jsonContent(await client.delete(`/iscsi/target/id/${id}`, force !== undefined ? { force } : undefined));
    });
    // =========================================================================
    // iSCSI — Extents (LUNs)
    // =========================================================================
    server.tool("iscsi_extent_list", "List all iSCSI extents (LUNs)", {}, async () => jsonContent(await client.get("/iscsi/extent")));
    server.tool("iscsi_extent_create", "Create an iSCSI extent (LUN)", {
        ...
        const body = clean(params);
        return jsonContent(await client.post("/iscsi/extent", body));
    });
    server.tool("iscsi_extent_update", "Update an iSCSI extent (LUN)", {
        ...
        const body = clean(rest);
        return jsonContent(await client.put(`/iscsi/extent/id/${id}`, body));
    });
    server.tool("iscsi_extent_delete", "Delete an iSCSI extent (LUN)", {
        id: z.number().describe("Extent ID"),
        ...
        if (!confirm)
            return jsonContent({ error: "Deletion not confirmed" });
        const body = clean({ remove, force });
        return jsonContent(await client.delete(`/iscsi/extent/id/${id}`, Object.keys(body).length ? body : undefined));
    });
    // =========================================================================
    // iSCSI — Portals
    // =========================================================================
    server.tool("iscsi_portal_list", "List all iSCSI portals", {}, async () => jsonContent(await client.get("/iscsi/portal")));
    server.tool("iscsi_portal_create", "Create an iSCSI portal", {
        comment: z.string().optional().describe("Description / comment"),
        ...
        const body = clean(params);
        return jsonContent(await client.post("/iscsi/portal", body));
    });
    server.tool("iscsi_portal_update", "Update an iSCSI portal", {
        ...
        const body = clean(rest);
        return jsonContent(await client.put(`/iscsi/portal/id/${id}`, body));
    });
    server.tool("iscsi_portal_delete", "Delete an iSCSI portal", {
        id: z.number().describe("Portal ID"),
        confirm: z.boolean().describe("Must be true to confirm deletion"),
    }, async ({ id, confirm }) => {
        if (!confirm)
            return jsonContent({ error: "Deletion not confirmed" });
        return jsonContent(await client.delete(`/iscsi/portal/id/${id}`));
    });
    // =========================================================================
    // iSCSI — Initiators
    // =========================================================================
    server.tool("iscsi_initiator_list", "List all iSCSI initiator groups", {}, async () => jsonContent(await client.get("/iscsi/initiator")));
    server.tool("iscsi_initiator_create", "Create an iSCSI initiator group", {
        comment: z.string().optional().describe("Description / comment"),
        initiators: z.array(z.string()).optional().describe("Allowed initiator IQNs (empty = allow all)"),
    }, async (params) => {
        const body = clean(params);
        return jsonContent(await client.post("/iscsi/initiator", body));
    });
    server.tool("iscsi_initiator_delete", "Delete an iSCSI initiator group", {
        id: z.number().describe("Initiator group ID"),
        confirm: z.boolean().describe("Must be true to confirm deletion"),
    }, async ({ id, confirm }) => {
        if (!confirm)
            return jsonContent({ error: "Deletion not confirmed" });
        return jsonContent(await client.delete(`/iscsi/initiator/id/${id}`));
    });
    // =========================================================================
    // iSCSI — Target-Extent Mappings
    // =========================================================================
    server.tool("iscsi_targetextent_list", "List all iSCSI target-to-extent mappings", {}, async () => jsonContent(await client.get("/iscsi/targetextent")));
    server.tool("iscsi_targetextent_create", "Create an iSCSI target-to-extent mapping", {
        ...
        const body = clean(params);
        return jsonContent(await client.post("/iscsi/targetextent", body));
    });
    server.tool("iscsi_targetextent_delete", "Delete an iSCSI target-to-extent mapping", {
        ...
        if (!confirm)
            return jsonContent({ error: "Deletion not confirmed" });
        return jsonContent(await client.delete(`/iscsi/targetextent/id/${id}`, force !== undefined ? { force } : undefined));
    });
    // =========================================================================
    // iSCSI — Sessions
    // =========================================================================
    server.tool("iscsi_sessions", "Get active iSCSI sessions", {}, async () => jsonContent(await client.get("/iscsi/global/sessions")));
}
