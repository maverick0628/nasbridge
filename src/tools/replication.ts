...
export function register(server, client) {
    ...
    // REPLICATION
    // =========================================================================
    server.tool("replication_list", "List all replication tasks", {}, async () => jsonContent(await client.get("/replication")));
    server.tool("replication_get", "Get a replication task by ID", { id: z.number().describe("Replication task ID") }, async ({ id }) => jsonContent(await client.get(`/replication/id/${id}`)));
    server.tool("replication_create", "Create a new replication task", {
        ...
        const result = await client.post("/replication", clean(params));
        return jsonContent(result);
    });
    server.tool("replication_update", "Update an existing replication task", {
        ...
        const result = await client.put(`/replication/id/${id}`, clean(rest));
        return jsonContent(result);
    });
    server.tool("replication_delete", "Delete a replication task (destructive — requires confirm)", {
        id: z.number().describe("Replication task ID"),
        confirm: z.boolean().describe("Must be true to confirm deletion"),
    }, async ({ id, confirm }) => {
        if (!confirm)
            return jsonContent({ error: "Deletion not confirmed. Set confirm to true." });
        const result = await client.delete(`/replication/id/${id}`);
        return jsonContent(result);
    });
    server.tool("replication_run", "Manually run a replication task now", { id: z.number().describe("Replication task ID") }, async ({ id }) => jsonContent(await client.post(`/replication/id/${id}/run`)));
    server.tool("replication_restore", "Restore from a replication task", {
        id: z.number().describe("Replication task ID"),
    ...
    }, async ({ id, ...rest }) => jsonContent(await client.post(`/replication/id/${id}/restore`, rest)));
    // =========================================================================
    // CLOUD SYNC
    // =========================================================================
    server.tool("cloudsync_list", "List all cloud sync tasks", {}, async () => jsonContent(await client.get("/cloudsync")));
    server.tool("cloudsync_get", "Get a cloud sync task by ID", { id: z.number().describe("Cloud sync task ID") }, async ({ id }) => jsonContent(await client.get(`/cloudsync/id/${id}`)));
    server.tool("cloudsync_create", "Create a new cloud sync task", {
    ...
    }, async (params) => jsonContent(await client.post("/cloudsync", clean(params))));
    server.tool("cloudsync_update", "Update an existing cloud sync task", {
    ...
    }, async ({ id, ...rest }) => jsonContent(await client.put(`/cloudsync/id/${id}`, clean(rest))));
    server.tool("cloudsync_delete", "Delete a cloud sync task (destructive — requires confirm)", {
        id: z.number().describe("Cloud sync task ID"),
        confirm: z.boolean().describe("Must be true to confirm deletion"),
    }, async ({ id, confirm }) => {
        if (!confirm)
            return jsonContent({ error: "Deletion not confirmed. Set confirm to true." });
        return jsonContent(await client.delete(`/cloudsync/id/${id}`));
    });
    server.tool("cloudsync_run", "Run a cloud sync task now", { id: z.number().describe("Cloud sync task ID") }, async ({ id }) => jsonContent(await client.post(`/cloudsync/id/${id}/sync`)));
    server.tool("cloudsync_abort", "Abort a running cloud sync task", { id: z.number().describe("Cloud sync task ID") }, async ({ id }) => jsonContent(await client.post(`/cloudsync/id/${id}/abort`)));
    server.tool("cloudsync_restore", "Restore data from cloud to a local path", {
    ...
    }, async ({ id, ...rest }) => jsonContent(await client.post(`/cloudsync/id/${id}/restore`, rest)));
    server.tool("cloudsync_providers", "List available cloud sync providers", {}, async () => jsonContent(await client.get("/cloudsync/providers")));
    // -- Cloud Sync Credentials ------------------------------------------------
    server.tool("cloudsync_credentials_list", "List all cloud sync credentials", {}, async () => jsonContent(await client.get("/cloudsync/credentials")));
    server.tool("cloudsync_credentials_create", "Create a new cloud sync credential", {
    ...
    }, async (params) => jsonContent(await client.post("/cloudsync/credentials", params)));
    server.tool("cloudsync_credentials_update", "Update an existing cloud sync credential", {
        ...
        attributes: z.record(z.string(), z.unknown()).optional().describe("Provider-specific attributes"),
    }, async ({ id, ...rest }) => jsonContent(await client.put(`/cloudsync/credentials/id/${id}`, clean(rest))));
    server.tool("cloudsync_credentials_delete", "Delete a cloud sync credential", { id: z.number().describe("Credential ID") }, async ({ id }) => jsonContent(await client.delete(`/cloudsync/credentials/id/${id}`)));
    server.tool("cloudsync_credentials_verify", "Verify a cloud sync credential is working", {
    ...
    }, async (params) => jsonContent(await client.post("/cloudsync/credentials/verify", params)));
    server.tool("cloudsync_list_buckets", "List remote buckets for a cloud credential", {
        credentials: z.number().describe("Credential ID"),
        attributes: z.record(z.string(), z.unknown()).optional().describe("Provider-specific attributes"),
    }, async (params) => jsonContent(await client.post("/cloudsync/list_buckets", clean(params))));
    server.tool("cloudsync_list_directory", "List files in a remote directory", {
        credentials: z.number().describe("Credential ID"),
        attributes: z.record(z.string(), z.unknown()).optional().describe("Provider-specific attributes"),
        id: z.number().optional().describe("Cloud sync task ID for context"),
    }, async (params) => jsonContent(await client.post("/cloudsync/list_directory", clean(params))));
    // =========================================================================
    // CLOUD BACKUP
    // =========================================================================
    server.tool("cloud_backup_list", "List all cloud backup tasks", {}, async () => jsonContent(await client.get("/cloud_backup")));
    server.tool("cloud_backup_create", "Create a new cloud backup task", {
    ...
    }, async (params) => jsonContent(await client.post("/cloud_backup", clean(params))));
    server.tool("cloud_backup_update", "Update an existing cloud backup task", {
        ...
        keep_last: z.number().optional().describe("Number of recent backups to keep"),
    }, async ({ id, ...rest }) => jsonContent(await client.put(`/cloud_backup/id/${id}`, clean(rest))));
    server.tool("cloud_backup_delete", "Delete a cloud backup task (destructive — requires confirm)", {
        id: z.number().describe("Cloud backup task ID"),
        confirm: z.boolean().describe("Must be true to confirm deletion"),
    }, async ({ id, confirm }) => {
        if (!confirm)
            return jsonContent({ error: "Deletion not confirmed. Set confirm to true." });
        return jsonContent(await client.delete(`/cloud_backup/id/${id}`));
    });
    server.tool("cloud_backup_run", "Run a cloud backup task now", { id: z.number().describe("Cloud backup task ID") }, async ({ id }) => jsonContent(await client.post(`/cloud_backup/id/${id}/sync`)));
    server.tool("cloud_backup_abort", "Abort a running cloud backup task", { id: z.number().describe("Cloud backup task ID") }, async ({ id }) => jsonContent(await client.post(`/cloud_backup/id/${id}/abort`)));
    server.tool("cloud_backup_snapshots", "List snapshots for a cloud backup task", { id: z.number().describe("Cloud backup task ID") }, async ({ id }) => jsonContent(await client.post(`/cloud_backup/id/${id}/list_snapshots`)));
    // =========================================================================
    // CRON JOBS
    // =========================================================================
    server.tool("cronjob_list", "List all cron jobs", {}, async () => jsonContent(await client.get("/cronjob")));
    server.tool("cronjob_create", "Create a new cron job", {
        ...
        schedule: z.object(scheduleSchema).optional().describe("Cron schedule"),
    }, async (params) => jsonContent(await client.post("/cronjob", clean(params))));
    server.tool("cronjob_update", "Update an existing cron job", {
        ...
        enabled: z.boolean().optional().describe("Enable this cron job"),
        schedule: z.object(scheduleSchema).optional().describe("Cron schedule"),
    }, async ({ id, ...rest }) => jsonContent(await client.put(`/cronjob/id/${id}`, clean(rest))));
    server.tool("cronjob_delete", "Delete a cron job", { id: z.number().describe("Cron job ID") }, async ({ id }) => jsonContent(await client.delete(`/cronjob/id/${id}`)));
    server.tool("cronjob_run", "Run a cron job immediately", { id: z.number().describe("Cron job ID") }, async ({ id }) => jsonContent(await client.post(`/cronjob/id/${id}/run`)));
    // =========================================================================
    // RSYNC TASKS
    // =========================================================================
    server.tool("rsync_task_list", "List all rsync tasks", {}, async () => jsonContent(await client.get("/rsynctask")));
    server.tool("rsync_task_create", "Create a new rsync task", {
    ...
    }, async (params) => jsonContent(await client.post("/rsynctask", clean(params))));
    server.tool("rsync_task_update", "Update an existing rsync task", {
        ...
        schedule: z.object(scheduleSchema).optional().describe("Cron schedule"),
        enabled: z.boolean().optional().describe("Enable this task"),
    }, async ({ id, ...rest }) => jsonContent(await client.put(`/rsynctask/id/${id}`, clean(rest))));
    server.tool("rsync_task_delete", "Delete an rsync task", { id: z.number().describe("Rsync task ID") }, async ({ id }) => jsonContent(await client.delete(`/rsynctask/id/${id}`)));
    server.tool("rsync_task_run", "Run an rsync task immediately", { id: z.number().describe("Rsync task ID") }, async ({ id }) => jsonContent(await client.post(`/rsynctask/id/${id}/run`)));
    // =========================================================================
    // INIT/SHUTDOWN SCRIPTS
    // =========================================================================
    server.tool("initshutdown_list", "List all init/shutdown scripts", {}, async () => jsonContent(await client.get("/initshutdownscript")));
    server.tool("initshutdown_create", "Create an init/shutdown script", {
    ...
    }, async (params) => jsonContent(await client.post("/initshutdownscript", clean(params))));
    server.tool("initshutdown_update", "Update an init/shutdown script", {
        ...
        timeout: z.number().optional().describe("Timeout in seconds"),
        comment: z.string().optional().describe("Comment / description"),
    }, async ({ id, ...rest }) => jsonContent(await client.put(`/initshutdownscript/id/${id}`, clean(rest))));
    server.tool("initshutdown_delete", "Delete an init/shutdown script", { id: z.number().describe("Init/shutdown script ID") }, async ({ id }) => jsonContent(await client.delete(`/initshutdownscript/id/${id}`)));
    // =========================================================================
    // SSH KEYPAIR / KEYCHAIN CREDENTIALS
    // =========================================================================
    server.tool("keychaincredential_list", "List all SSH credentials and keypairs", {}, async () => jsonContent(await client.get("/keychaincredential")));
    server.tool("keychaincredential_create", "Create an SSH credential or keypair", {
        name: z.string().describe("Credential name"),
    ...
    }, async (params) => jsonContent(await client.post("/keychaincredential/remote_ssh_host_key_scan", clean(params))));
}
