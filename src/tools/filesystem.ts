...
export function register(server, client) {
    ...
    // Filesystem
    // ---------------------------------------------------------------------------
    server.tool("filesystem_stat", "Get file or directory info including permissions, size, owner, and timestamps. Provide the full path on the TrueNAS system.", {
    ...
    server.tool("filesystem_listdir", "List contents of a directory. Returns files and subdirectories with metadata. Supports pagination via limit and offset.", {
    ...
    server.tool("filesystem_mkdir", "Create a new directory at the specified path. Optionally set the UNIX mode (permissions).", {
    ...
    server.tool("filesystem_set_permissions", "Set UNIX permissions on a file or directory. Can optionally apply recursively and strip existing ACLs.", {
    ...
    server.tool("filesystem_get_acl", "Get the Access Control List (ACL) for a file or directory path. Optionally return a simplified representation.", {
        path: z.string().describe("Full filesystem path"),
    ...
    server.tool("filesystem_set_acl", "Set the Access Control List (ACL) for a file or directory. This is a powerful operation — the 'confirm' parameter must be true to proceed.", {
        ...
        const body = { path, dacl };
        if (nfs41_flags !== undefined)
            body.nfs41_flags = nfs41_flags;
        if (acltype !== undefined)
            body.acltype = acltype;
        if (uid !== undefined)
            body.uid = uid;
        if (gid !== undefined)
            body.gid = gid;
    ...
    server.tool("filesystem_chown", "Change ownership of a file or directory. Can optionally apply recursively.", {
        path: z.string().describe("Full filesystem path"),
    ...
    // ---------------------------------------------------------------------------
    server.tool("reporting_config", "Get the current reporting/metrics configuration.", {}, async () => {
        const result = await client.get("/reporting");
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
    });
    server.tool("reporting_graphs", "List all available reporting graphs (CPU, memory, disk, network, etc.). Use the graph names with reporting_get_data to fetch time-series data.", {}, async () => {
        const result = await client.post("/reporting/netdata_graphs");
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
    });
    server.tool("reporting_get_data", "Get time-series reporting data for one or more graphs (CPU, memory, disk, network, etc.). Use reporting_graphs first to discover available graph names and identifiers.", {
        graphs: z
            .array(z.object({
        ...
        if (start !== undefined)
            reporting_query.start = start;
        if (end !== undefined)
            reporting_query.end = end;
    ...
    // Directory Services
    // ---------------------------------------------------------------------------
    server.tool("directory_services_config", "Get directory services configuration (Active Directory / LDAP).", {}, async () => {
        const result = await client.get("/directoryservices");
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
    });
    server.tool("directory_services_update", "Update directory services configuration. Fields depend on whether Active Directory or LDAP is configured. Pass only the fields you want to change.", {
    ...
    server.tool("directory_services_status", "Get current directory services status including connection state and health.", {}, async () => {
        const result = await client.get("/directoryservices/status");
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
    });
    server.tool("directory_services_leave", "Leave the current Active Directory or LDAP domain. This is a DESTRUCTIVE operation — 'confirm' must be true to proceed. Requires domain credentials.", {
    ...
    server.tool("directory_services_cache_refresh", "Refresh the directory services cache. Forces re-read of users and groups from the directory server.", {}, async () => {
        const result = await client.post("/directoryservices/cache_refresh");
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
    });
    server.tool("kerberos_config", "Get Kerberos configuration settings.", {}, async () => {
        const result = await client.get("/kerberos");
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
    });
    server.tool("kerberos_realm_list", "List all configured Kerberos realms.", {}, async () => {
        const result = await client.get("/kerberos/realm");
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
    });
    server.tool("kerberos_keytab_list", "List all configured Kerberos keytabs.", {}, async () => {
    ...
    // ---------------------------------------------------------------------------
    server.tool("tunable_list", "List all system tunables (sysctl, loader, and rc variables).", {}, async () => {
        const result = await client.get("/tunable");
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
    });
    server.tool("tunable_create", "Create a new system tunable. Tunables allow setting sysctl, loader, or rc variables.", {
    ...
    server.tool("tunable_update", "Update an existing tunable by its ID. Use tunable_list to find the ID. All fields are optional — only provide the ones to change.", {
    ...
    server.tool("tunable_delete", "Delete a tunable by its ID. Use tunable_list to find the ID.", {
    ...
    // Service Configs — SSH
    // ---------------------------------------------------------------------------
    server.tool("ssh_config", "Get the current SSH service configuration.", {}, async () => {
        const result = await client.get("/ssh");
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
    });
    server.tool("ssh_config_update", "Update SSH service configuration. All fields are optional — only provide the ones you want to change. Restart the SSH service after changes.", {
        ...
        const body = {};
        if (params.tcpport !== undefined)
            body.tcpport = params.tcpport;
        if (params.rootlogin !== undefined)
            body.rootlogin = params.rootlogin;
        if (params.passwordauth !== undefined)
            body.passwordauth = params.passwordauth;
        if (params.kerberosauth !== undefined)
            body.kerberosauth = params.kerberosauth;
        if (params.tcpfwd !== undefined)
            body.tcpfwd = params.tcpfwd;
        if (params.bindiface !== undefined)
            body.bindiface = params.bindiface;
        if (params.compression !== undefined)
            body.compression = params.compression;
        if (params.sftp_log_level !== undefined)
            body.sftp_log_level = params.sftp_log_level;
        if (params.sftp_log_facility !== undefined)
            body.sftp_log_facility = params.sftp_log_facility;
        if (params.weak_ciphers !== undefined)
            body.weak_ciphers = params.weak_ciphers;
    ...
    // Service Configs — FTP
    // ---------------------------------------------------------------------------
    server.tool("ftp_config", "Get the current FTP service configuration.", {}, async () => {
        const result = await client.get("/ftp");
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
    });
    server.tool("ftp_config_update", "Update FTP service configuration. All fields are optional — only provide the ones you want to change. Restart the FTP service after changes.", {
    ...
    // ---------------------------------------------------------------------------
    server.tool("snmp_config", "Get the current SNMP service configuration.", {}, async () => {
        const result = await client.get("/snmp");
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
    });
    server.tool("snmp_config_update", "Update SNMP service configuration. All fields are optional — only provide the ones you want to change.", {
    ...
    // Service Configs — UPS
    // ---------------------------------------------------------------------------
    server.tool("ups_config", "Get the current UPS service configuration.", {}, async () => {
        const result = await client.get("/ups");
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
    });
    server.tool("ups_config_update", "Update UPS service configuration. All fields are optional — only provide the ones you want to change.", {
    ...
    // Privilege & Audit
    // ---------------------------------------------------------------------------
    server.tool("privilege_list", "List all privileges/roles configured on the system.", {}, async () => {
        const result = await client.get("/privilege");
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
    });
    server.tool("privilege_create", "Create a new privilege/role with specific permissions and group bindings.", {
    ...
    server.tool("privilege_update", "Update an existing privilege/role by its ID. Use privilege_list to find the ID. All fields are optional.", {
    ...
    server.tool("privilege_delete", "Delete a privilege/role by its ID. Use privilege_list to find the ID.", {
        id: z.number().describe("Privilege ID to delete"),
    }, async ({ id }) => {
        const result = await client.delete(`/privilege/id/${id}`);
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
    });
    server.tool("audit_query", "Query the TrueNAS audit log. Supports filtering by services and applying query filters and options.", {
    ...
    server.tool("audit_config", "Get the current audit configuration including retention and quota settings.", {}, async () => {
        const result = await client.get("/audit");
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
    });
    server.tool("audit_config_update", "Update audit configuration. All fields are optional — only provide the ones you want to change.", {
    ...
    // ---------------------------------------------------------------------------
    server.tool("truenas_api_call", "Make a raw API call to any TrueNAS endpoint not covered by specific tools. This is an escape hatch for advanced or uncommon operations. The body parameter accepts a JSON string that will be parsed before sending.", {
        method: z.enum(["GET", "POST", "PUT", "DELETE"]).describe("HTTP method"),
        path: z.string().describe("API path, e.g. '/pool/dataset'"),
        body: z.string().optional().describe("Request body as a JSON string (optional, parsed before sending)"),
        ...
        if (body !== undefined) {
            try {
                ...
                return {
                    content: [{ type: "text", text: `Failed to parse body as JSON: ${e.message}` }],
                };
            }
        }
        let result;
