        ...
        for (const action of actions) {
            ...
            if (params.length > 0) {
                ...
                if (required.length > 0) {
                    lines.push(`    Required: ${required.map((p) => `${p.name}${p.description ? ` (${p.description})` : ""}`).join(", ")}`);
                }
                if (optional.length > 0) {
                    lines.push(`    Optional: ${optional.map((p) => `${p.name}${p.description ? ` (${p.description})` : ""}`).join(", ")}`);
                }
            }
        }
        return lines.join("\n");
...
export function categorize(name: string): Category {
    ...
    if (name.startsWith("vm_"))
        return "vm";
    // Apps
    if (name.startsWith("app_") || name.startsWith("docker_"))
        return "app";
    // Update & Boot
    ...
    if (name.startsWith("filesystem_"))
        return "filesystem";
    // Reporting
    if (name.startsWith("reporting_"))
        return "reporting";
    // Directory Services
    if (name.startsWith("directory_") || name.startsWith("kerberos_"))
        return "directory";
    // Service Configs
    if (name.startsWith("ssh_config") ||
...
function extractParams(schema: ToolSchema): { name: string; required: boolean; description: string }[] {
    ...
    for (const [name, field] of Object.entries(schema)) {
        ...
        try {
            if (typeof f.isOptional === "function" && f.isOptional()) {
                required = false;
            }
        }
        catch {
